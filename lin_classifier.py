...
def cv_kfold(X, y, C, penalty, K, mode):
    ...
    for c in C:
        ...
        # ------------------ IMPLEMENT YOUR CODE HERE:-----------------------------
                ...
                loss = log_loss(y[val_idx], y_pred)
                loss_val_vec[k] = loss
                k += 1
            # after K iterations of splits
            mean_loss = loss_val_vec.mean()
            std_loss = np.std(loss_val_vec)
            dict_ = {'C': c, 'penalty': p, 'mu': mean_loss, 'sigma': std_loss}
            validation_dict.append(dict_)  # list of dicts
        # --------------------------------------------------------------------------
